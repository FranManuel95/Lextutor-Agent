...
export async function generateResponseWithContext(
    ...
    if (run.usage) {
        ...
        const INPUT_COST_PER_1M = 0.150;   // $0.150 / 1M input tokens
        const OUTPUT_COST_PER_1M = 0.600;  // $0.600 / 1M output tokens
        ...
        console.log(`   📤 Output: ${completion_tokens.toLocaleString()} tokens (€${outputCost.toFixed(6)})`);
    ...
    const annotations = assistantMessage.content[0].type === "text"
