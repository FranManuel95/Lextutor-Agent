...
export const POST = createApiHandler(
    async ({ user, supabase, body }) => {
        ...
        if ((!chat.title || chat.title === "Nuevo Chat") && message.trim().length > 3) {
            ...
            if (newTitle) {
                // Non-blocking update
                supabase.from("chats").update({ title: newTitle } as any).eq("id", chatId).then();
            }
        }
        ...
        // We don't await savePromise to reduce latency for the user
        // Next.js runtime will keep the process alive long enough usually, 
        // but for critical stuff we might want to ensure it finishes if needed.
        // For standard serverless/edge, you'd use waitUntil (not available here in nodejs easily without complications)
        // Since we are in nodejs runtime, the loop stays open.

        return {
            response: assistantResponse,
            grounding: null // We now embed grounding in the response text in ai-service
        };
    },
    {
        schema: chatSchema,
