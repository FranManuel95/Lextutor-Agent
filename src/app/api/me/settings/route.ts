...
const settingsSchema = z.object({
    ...
    modes: z.array(z.enum(['concise', 'feynman', 'quiz', 'socratic', 'steps', 'citations', 'caseMode', 'memoryAware'])).max(8).optional(),
...
export async function PATCH(request: NextRequest) {
    ...
    try {
        ...
        const { data: profile } = await supabase.from('profiles').select('tutor_prefs').eq('id', user.id).single()
