...
import { useAppStore, TutorSettings } from "@/store/useAppStore";
...
export function SettingsDialog() {
  ...
  const toggleMode = (mode: TutorSettings["modes"][number]) => {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className="text-gem-muted hover:bg-gem-offwhite/10 hover:text-law-gold"
        >
          <Settings2 size={20} />
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-lg border-law-accent/20 bg-gem-onyx text-gem-offwhite">
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(v: TutorSettings["area"]) =>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 gap-2">
              {modes.map((mode) => {
                const isActive = localSettings.modes.includes(mode.id);
                return (
                  <div
                    key={mode.id}
        ...
        <div className="flex justify-between gap-3 pt-2">
          ...
          <Button
            onClick={handleSave}
            disabled={loading}
            className="min-w-[120px] bg-law-gold font-bold text-gem-onyx hover:bg-law-gold/90"
          >
            {loading ? (
              "Guardando..."
            ) : (
